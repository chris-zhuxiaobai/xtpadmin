            ...
            <div class="col-md-10 col-md-offset-1">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form class="form-horizontal" role="form" method="POST" action="{{ url('tradeway') }}">
                            {{ csrf_field() }}
                                ...
                                <div class="col-md-8">
                                    <select name="type{{$type}}" id="type{{$type}}" class="form-control select" >
                                        <option value="0" > ---请选择--- </option>
                                            @foreach($rows as $row)
                                                <option value="{{$row->id}}" @if ($row->id==old('type'.$type)) selected @endif>{{$row->name}}</option>
                                            @endforeach
                                    </select>
